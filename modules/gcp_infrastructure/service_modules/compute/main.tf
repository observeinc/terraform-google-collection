...
resource "google_compute_instance" "instances" {
  ...
  tags = ["externalssh", "content", "arthur", "network-lb-tag"]
  ...
  # python app
  # provisioner "file" {
  #   source      = "${path.module}/flask/main.py"
  #   destination = "/home/${each.value.default_user}/flask/main.py"
  #   connection {
  #     type        = "ssh"
  #     user        = each.value.default_user
  #     private_key = file(var.private_key_path)
  ...
  # # python app
  # provisioner "file" {
  #   source      = "${path.module}/flask/templates"
  #   destination = "/home/${each.value.default_user}/flask"
  #   connection {
  #     type        = "ssh"
  #     user        = each.value.default_user
  #     private_key = file(var.private_key_path)
  ...
  # provisioner "remote-exec" {
  #   inline = [
  #     var.observe.install_linux_host_monitoring == true ? "sleep ${each.value.wait}; curl \"https://raw.githubusercontent.com/observeinc/linux-host-configuration-scripts/main/observe_configure_script.sh\" | bash -s -- --customer_id \"${var.observe.customer_id}\" --ingest_token \"${var.observe.datastream_token}\" --observe_host_name https://${var.observe.customer_id}.collect.${var.observe.domain}/ --config_files_clean TRUE --ec2metadata FALSE --datacenter GCP --appgroup MY_APP_GROUP" : "ls;"
  #   ]
